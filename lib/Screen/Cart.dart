...
import 'package:eshop_multivendor/Provider/SettingProvider.dart';
import 'package:eshop_multivendor/Provider/UserProvider.dart';
import 'package:eshop_multivendor/Screen/deliveryToAddress.dart';
import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:flutter_paystack/flutter_paystack.dart';
import 'package:flutter_svg/svg.dart';
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
      ...
      child: Scaffold(
        backgroundColor: colors.blackTemp,
          appBar :  AppBar(
            ...
            leadingWidth: 95, //TODO Adjust leading container width
            leading: Center(
                child: Text(
                  'CART',
  ...
  Widget listItem(int index, List<SectionModel> cartList) {
    ...
    for(int j=0;j<cartList[index].productList![0].addOnList!.length;j++){
      ...
      print("dars"+model.price.toString()+cartList[index].add_on_id.toString());
            ...
            Row(
              ...
              children: [
               ...
               /* Text(
                ...
                Text(
                    // "${cartList[index].addOns![i].totalAmount.toString()}",
                 "${cartList[index].addOns!.map((e) =>  e.totalAmount)}",
                  style: TextStyle(
                      color: Theme.of(context).colorScheme.lightBlack2),
                )
              ],
    ...
    if (_isNetworkAvail) {
      try {
        ...
        //  dCharge = double.parse(getdata['delivery_charge'].toString());
         ...
         print("qqqqqqqqqqqqqqqqqqq---${cartList}");
          if (getdata.containsKey(PROMO_CODES)) {
            var promo = getdata[PROMO_CODES];
            promoList =
                (promo as List).map((e) => new Promo.fromJson(e)).toList();
        ...
        child: Column(
          ...
          children: [
            ...
            addressList.length > 0
              ...
              Divider(),
            deliveryAddres == null || deliveryAddres == ""  ? SizedBox.shrink() :        Text("Deliver to :"),
              SizedBox(height: 10,),
            deliveryAddres == null  ?  InkWell(
                onTap: ()async{
                final result = await Navigator.push(context, MaterialPageRoute(builder: (context) => DeliverToPage()));
                ...
                child: Container(
                  ...
                  alignment: Alignment.center,
                    child: Text("Add delivery address",style: TextStyle(color: Colors.white,fontSize: 14,fontWeight: FontWeight.w500),),
                ),
              ) : SizedBox.shrink(),
            deliveryAddres == null || deliveryAddres == ""  ? SizedBox.shrink(): Text("${deliveryAddres['name']}\n ${deliveryAddres['address']}, ${deliveryAddres['email']}, ${deliveryAddres['mobile1']}, ${deliveryAddres['mobile2']}, ${deliveryAddres['type']} "),
          ],
        ),
      ),
    );
