...
/*  _transactionscrollListener() {
  ...
  Widget build(BuildContext context) {
    print("final check here ${widget.model!.addOnList} and ${widget.model!.addOnList!.length}");
                ...
                children: [
                              ...
                              children: [
                                ChoiceChip(
                                  selected: addIdList.indexWhere(
                                          (element) => element.id == model.addOnList[i].id!) !=
                                      -1
                                      ? true
                                      : false,
                                  label: Row(
                                    children: [
                                      Icon(
                                        ...
                                        element.id == model.addOnList[i].id!) !=
                                            -1
                                            ? Icons.check_box
                                            : Icons.check_box_outline_blank,
                                        color: addIdList.contains(model.addOnList[i].id)
                                            ? Colors.green
                                            : Colors.grey,
                                      ),
                                      SizedBox(
                                      ...
                                      Text(model.addOnList[i].name.toString(),
                                          style: TextStyle(
                                      ...
                                      Text(
                                        CUR_CURRENCY! + " " + model.addOnList[i].price.toString(),
                                  ...
                                  onSelected: (bool selected) {
                                    ...
                                    if (selected) {
                                      if (mounted) {
                                        persistentBottomSheetController!
                                            .setState!(() {
                                          model.addOnList[i].cartCount = "1";
                                          addIdList.add(AddQtyModel(model.addOnList[i].id!, "1"));
                                          print("addIdList data here");
                                          for(var i=0;i<addIdList.length;i++){
                                            print("result here now ${addIdList[i].id} and ${addIdList[i].qty}");
                                          }
                                          priceAdd += double.parse(model.addOnList[i].price!);
                                        });
                                        if (_controller.length < i + 1)
                                          _controller.add(new TextEditingController());
                                      }
                                  ...
                                  children: [
                                    IconButton(
                                        onPressed: () {
                                          persistentBottomSheetController!
                                              .setState!(() {
                                            print(model.addOnList[i].cartCount);
                                            model.addOnList[i].cartCount =
                                                (int.parse(model.addOnList[i].cartCount!) +
                                                    1)
                                                    .toString();
                                            if (addIdList.indexWhere((element) =>
                                            element.id == model.addOnList[i].id!) !=
                                              ...
                                              addIdList[addIdList.indexWhere(
                                                      (element) => element.id == model.addOnList[i].id!)].qty=model.addOnList[i].cartCount!;
                                            }
                                            priceAdd += double.parse(model.addOnList[i].price!);
                                          });
                                        },
                                    ...
                                    Text(model.addOnList[i].cartCount.toString(),
                                        style: TextStyle(
                                            fontWeight: FontWeight.w500,
                                            fontSize: 15,
                                            color: Theme.of(context)
                                    ...
                                    IconButton(
                                        onPressed: () {
                                          persistentBottomSheetController!
                                              .setState!(() {
                                            if (model.addOnList[i].cartCount != "1") {
                                              model.addOnList[i].cartCount =
                                                  (int.parse(model.addOnList[i].cartCount!) -
                                                      1)
                                                      .toString();
                                              ...
                                              priceAdd -=
                                                  double.parse(model.addOnList[i].price!);
                                            }
                                          });
                                        },
                                        icon: Icon(
                  ...
                  !loading
                    ...
                    onPressed: available
                      ...
                      //applyVarient();
                      var index;
                      print("addon list here ${addIdList.length}");
                      for(var i=0;i<addIdList.length;i++){
                        index = i;
                        print("hello here ${index}");
                      }
                      print("indexxxxxxxxxx ${index}");

                      applyVarient1(
                          model, widget.index, addIdList.toList());
                    }
                        : null,
...
*/
  ...
  Future<void> addToCart(String qty, bool intent,  List<AddQtyModel> addList) async {
    List fileDate = [];
    if(widget.model!.is_customize == "1") {
      print("yes show toast");
       if(fileDate.isEmpty){
         Fluttertoast.showToast(msg: 'Please upload image first');
       }
       else{
         fileDate = aadharImage!.path.split(
             "/data/user/0/com.giftbash.user/cache/");
       }
    ...
    if (_isNetworkAvail) {
      if (CUR_USERID != null) {
        try {
          ...
          // List fileDate = aadharImage!.path.split("/data/user/0/com.giftbash.user/cache/");
          // print("pppppppapapappappa=====${fileDate}");
          print("hello here ${fileDate}");
          int index1 = 0;
          ...
          var parameter = {
            ...
            "add_on_qty": index1 != -1 && int.parse(qty) > 1
                ...
                : "",
            "image": fileDate.isEmpty ? "" : fileDate[1].toString()
          };
          var parameter1 = {
            USER_ID: CUR_USERID,
            PRODUCT_VARIENT_ID: model.prVarientList![model.selVarient!].id,
          ...
          print("aaaaaaaaaaaaaaaaaaaaaaaa====${parameter.toString()}");
          Response response =
          await post(manageCartApi, body: widget.model!.is_customize == "0" ? parameter1 : parameter, headers: headers)
              .timeout(Duration(seconds: timeOut));
          print("ookok ${manageCartApi}");
          print("SYREYEYEET======${response.body}");
                  ...
                  children: <Widget>[
                    Column(
                      ...
                      //  aadon(),
                       ...
                       widget.model!.is_customize == "0" ? SizedBox.shrink() : _images(),

                        showAddon == true ? _addOn() :SizedBox()
          ...
          children: [
            ...
            Expanded(
              child: TextButton.icon(
                  ...
                  onPressed: () {
                    if(widget.model!.is_customize == "0")
                      {
 ...
 _addOn(){
    ...
    return ListView.builder(
      shrinkWrap: true,
      itemCount:widget.model!.addOnList.length,
      physics:  NeverScrollableScrollPhysics(),
      itemBuilder: (context, index) {
        return  Padding(
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: ClipRRect(
                        borderRadius: BorderRadius.circular(10),
                        child: Image.network("${widget.model!.addOnList[index].image}",fit: BoxFit.fill,)
                    ),
                  ),
                  Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text("${widget.model!.addOnList[index].name}",style: TextStyle(color: Colors.black,fontSize: 16,fontWeight: FontWeight.w600),),
                        SizedBox(height: 10,),
                        Text("Price : $CUR_CURRENCY${widget.model!.addOnList[index].price}"),
                        SizedBox(height: 10,),
                        Card(
                          elevation: 2,
                          shape: RoundedRectangleBorder(
                            ...
                            child: Row(
                              ...
                              children: [
                                InkWell(
                                    onTap: (){
                                      setState(() {
                                        int.parse(_controller[index].text.toString()) - 1;
                                      });
                                      print("checking controller remove ${_controller[index]}");
                                    },
                                ...
                                InkWell(
                                    onTap: (){
                                      ...
                                      print("checking controller add ${_controller[index]}");
                                    },
          ...
          child: Row(
            children: [
              Container(
                ...
                child: ClipRRect(
                    borderRadius: BorderRadius.circular(10),
                    child: Image.network("${AddOnNew!.data![index].image}",fit: BoxFit.fill,)
                ),
              ),
              Padding(
                padding: const EdgeInsets.only(top: 15,left: 10),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("${AddOnNew!.data![index].name}",style: TextStyle(color: Colors.black,fontSize: 16,fontWeight: FontWeight.w600),),
                    SizedBox(height: 10,),
                    Text("Price : $CUR_CURRENCY${AddOnNew!.data![index].price}"),
                    SizedBox(height: 10,),
                    Card(
                      elevation: 2,
                      shape: RoundedRectangleBorder(
...
/*  AddOnModel?  AddOnNew;
  ...
  getAddOn() async {
    var headers = {
      'Cookie': 'ci_session=de436543cc1c090db19d97640ec671255af38715'
    };
    var request = http.MultipartRequest('POST', Uri.parse('$baseUrl/get_add_on_product'));
    ...
    if (response.statusCode == 200) {
      ...
      var finalData = AddOnModel.fromJson(jsonDecode(result));
      print("ADDDADDADADDDDADADDDADDA===${finalData.data}");
      setState(() {
        AddOnNew = finalData;
      });
    }
                ...
                child: Align(
                  alignment: Alignment.topRight ,
                    child: Container(
                        ...
                        child: Icon(Icons.clear,color: colors.whiteTemp,)
                    )
                ),
              )
            ],
